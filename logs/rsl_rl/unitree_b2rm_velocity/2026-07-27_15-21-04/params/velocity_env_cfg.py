...
class CommandsCfg:
    base_velocity = mdp.UniformLevelVelocityCommandCfg(
        ...
        debug_vis=True,
        # Match the initial B2RM Parkour walking curriculum: forward walking
        # and yaw first; lateral and reverse commands come after this baseline.
        ranges=mdp.UniformLevelVelocityCommandCfg.Ranges(
            lin_vel_x=(0.0, 0.60), lin_vel_y=(0.0, 0.0), ang_vel_z=(-0.40, 0.40)
        ),
...
@configclass
class RewardsCfg:
    # Flat-ground subset of the existing B2RM Parkour reward design.  It keeps
    # velocity tracking and gait stability, without terrain, target, or vision terms.
    ...
    dont_wait = RewTerm(func=mdp.dont_wait, weight=-2.0, params={"command_name": "base_velocity"})
    must_turn = RewTerm(
        func=mdp.must_turn,
        weight=-2.0,
    ...
    base_height = RewTerm(func=mdp.base_height_l2, weight=-4.0, params={"target_height": 0.63})
    ...
    air_time_variance = RewTerm(func=mdp.air_time_variance_penalty, weight=-1.0, params={"sensor_cfg": FEET_CFG})
    feet_slide = RewTerm(
